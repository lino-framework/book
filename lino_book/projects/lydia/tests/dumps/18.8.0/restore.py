...
courses_CourseType = resolve_model("courses.CourseType")
...
topics_TopicGroup = resolve_model("topics.TopicGroup")
...
def create_cal_eventtype(id, seqno, name, attach_to_email, email_template, description, is_appointment, all_rooms, locks_user, force_guest_states, start_date, event_label, max_conflicting, max_days, transparent, planner_column):
...
def create_courses_coursetype(id, ref, name):
    kw = dict()
    kw.update(id=id)
    kw.update(ref=ref)
    if name is not None: kw.update(bv2kw('name',name))
    return courses_CourseType(**kw)
...
def create_courses_line(id, ref, name, company_id, contact_person_id, contact_role_id, excerpt_title, course_area, topic_id, description, every_unit, every, event_type_id, fee_id, guest_role_id, options_cat_id, fees_cat_id, body_template, course_type_id):
...
#    if every_unit: every_unit = settings.SITE.models.cal.Recurrencies.get_by_value(every_unit)
    ...
    kw.update(body_template=body_template)
    kw.update(course_type_id=course_type_id)
    return courses_Line(**kw)

def create_households_household(partner_ptr_id, type_id, client_state, tariff):
#    if client_state: client_state = settings.SITE.models.clients.ClientStates.get_by_value(client_state)
#    if tariff: tariff = settings.SITE.models.tera.PartnerTariffs.get_by_value(tariff)
    kw = dict()
    kw.update(type_id=type_id)
...
def create_phones_contactdetail(id, detail_type, partner_id, value, remark, primary, end_date):
...
def create_system_siteconfig(id, default_build_method, simulate_today, site_company_id, next_partner_id, default_event_type_id, site_calendar_id, max_auto_events, hide_events_before, system_note_type_id):
...
def create_topics_topicgroup(id, name, description):
    kw = dict()
    kw.update(id=id)
    if name is not None: kw.update(bv2kw('name',name))
    kw.update(description=description)
    return topics_TopicGroup(**kw)

def create_topics_topic(id, ref, name, description, topic_group_id):
    kw = dict()
    kw.update(id=id)
    kw.update(ref=ref)
    if name is not None: kw.update(bv2kw('name',name))
    if description is not None: kw.update(bv2kw('description',description))
    kw.update(topic_group_id=topic_group_id)
    return topics_Topic(**kw)

def create_users_user(id, email, language, modified, created, start_date, end_date, password, last_login, username, user_type, initials, first_name, last_name, remarks, time_zone, access_class, event_type_id, team_id, partner_id):
...
def create_cal_task(id, modified, created, project_id, start_date, start_time, user_id, owner_type_id, owner_id, summary, description, access_class, sequence, auto_type, due_date, due_time, percent, state):
    owner_type_id = new_content_type_id(owner_type_id)
#    if access_class: access_class = settings.SITE.models.cal.AccessClasses.get_by_value(access_class)
...
def create_invoicing_item(id, plan_id, partner_id, first_date, last_date, amount, number_of_invoiceables, preview, selected, invoice_id):
    if amount is not None: amount = Decimal(amount)
    kw = dict()
    kw.update(id=id)
    kw.update(plan_id=plan_id)
    kw.update(partner_id=partner_id)
    kw.update(first_date=first_date)
    kw.update(last_date=last_date)
    kw.update(amount=amount)
    kw.update(number_of_invoiceables=number_of_invoiceables)
...
def create_sheets_report(id, user_id, today, printed_by_id, start_period_id, end_period_id):
    kw = dict()
    kw.update(id=id)
    kw.update(user_id=user_id)
    kw.update(today=today)
...
def create_tera_client(person_ptr_id, modified, created, user_id, client_state, private, starting_reason, ending_reason, professional_state, tariff, obsoletes_id, translator_type, unemployed_since, seeking_since, work_permit_suspended_until, declared_name, unavailable_until, unavailable_why, family_notes, residence_notes, health_notes, financial_notes, integration_notes, availability, needed_course_id, language_notes, nationality_id):
#    if client_state: client_state = settings.SITE.models.clients.ClientStates.get_by_value(client_state)
#    if starting_reason: starting_reason = settings.SITE.models.tera.StartingReasons.get_by_value(starting_reason)
#    if ending_reason: ending_reason = settings.SITE.models.tera.EndingReasons.get_by_value(ending_reason)
#    if professional_state: professional_state = settings.SITE.models.tera.ProfessionalStates.get_by_value(professional_state)
#    if tariff: tariff = settings.SITE.models.tera.PartnerTariffs.get_by_value(tariff)
#    if translator_type: translator_type = settings.SITE.models.tera.TranslatorTypes.get_by_value(translator_type)
    kw = dict()
    kw.update(modified=modified)
    kw.update(created=created)
    kw.update(user_id=user_id)
    ...
    kw.update(starting_reason=starting_reason)
    ...
    kw.update(family_notes=family_notes)
    kw.update(residence_notes=residence_notes)
    kw.update(health_notes=health_notes)
    kw.update(financial_notes=financial_notes)
    kw.update(integration_notes=integration_notes)
    kw.update(availability=availability)
    kw.update(needed_course_id=needed_course_id)
    kw.update(language_notes=language_notes)
    kw.update(nationality_id=nationality_id)
    return create_mti_child(contacts_Person, person_ptr_id, tera_Client, **kw)

def create_courses_enrolment(id, start_date, end_date, user_id, printed_by_id, course_area, course_id, pupil_id, request_date, state, places, option_id, remark, confirmation_details, fee_id):
#    if course_area: course_area = settings.SITE.models.courses.CourseAreas.get_by_value(course_area)
...
def main(args):
    ...
    execfile("courses_coursetype.py", *args)
