...
Maybe fixed :ticket:`2986` (Automatic browser update fails when using a permalink)
...
Fixed: :manage:`configure` failed when appy was needed but not yet installed.
...
The :manage:`configure` command didn't show the output of the subcommands.
Fixed.
...
Added a new admin command :manage:`configure`.
