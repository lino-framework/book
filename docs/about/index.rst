.. _lino.about:

==========
About Lino
==========

**Lino** is a high-level framework for writing desktop-like customized
database web applications based on `Django
<https://www.djangoproject.com/>`_ and and `Sencha ExtJS
<http://www.sencha.com/products/extjs/>`_.

A **database application** is a computer program whose primary purpose is
entering and retrieving information from a database (`Wikipedia
<https://en.wikipedia.org/wiki/Database_application>`__).
A **customized** database application is one which is taylored to your
needs. This means that you did not find any off-the-shelf application
which satisfies your needs. It implies that you are ready to invest
some of your time for analysis and testing of your software.
**Desktop-like** means that your application is optimized for being
used from a *desktop* computer and not from a *mobile* device. 

From the system administrator's point of view, Lino applications are
**just Django** projects.  Many administrators know how to host a
Django project, and these people can also host a Lino application.
Lino is written mainly in the `Python <https://www.python.org/>`_
programming language, but also contains Javascript code.

The **advantages** of Lino versus plain Django are visible for the
developer (technical details in :doc:`lino_and_django`), but these
advantages are indirectly tangible to their customers because many
parts of the development process like writing a prototype, adapting
your application to changed needs and providing long-term maintenance
become much easier (read: cheaper).

Lino is designed to be **used by professional** developers who write
and maintain such applications for their employer or their customers.
The growing collection of :ref:`lino.projects` can be used by
**service providers** who offer professional hosting of one of these
applications.

Primary **target users** of Lino applications are organizations who
need a customized database application "more than MS-Access for
cheaper than SAP".  Typical Lino applications have a rather **complex
database structure**.  For example Lino Welfare has 155 models in 65
plugins.  Even Lino Noi (the smallest Lino application which is being
used on a production site) has 44 models in 32 plugins.


.. toctree::
   :maxdepth: 1

   why
   features
   limitations
   more
   
